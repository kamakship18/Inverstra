...
contract FetchAIPredictionDAO is Ownable, ReentrancyGuard {
    struct Prediction {
        uint256 id;
        address creator;
        string title;
    ...
    mapping(uint256 => AIAnalysis[]) public aiAnalyses; // New: Store AI analyses
    mapping(string => bool) public authorizedAgents; // New: Authorize AI agents
    ...
    event AIAnalysisAdded(uint256 indexed predictionId, string indexed agentId, uint256 credibilityScore); // New event
    ...
    modifier onlyAuthorizedAgent(string memory agentId) { // New modifier
        require(authorizedAgents[agentId], "Unauthorized AI agent");
        _;
    }
    ...
    // New functions for AI agent integration
    function addAIAnalysis(
        uint256 _predictionId,
        string memory _agentId,
        uint256 _credibilityScore,
        string memory _marketContext,
        string memory _riskAssessment
    ) external onlyAuthorizedAgent(_agentId) {
        aiAnalyses[_predictionId].push(AIAnalysis({
        ...
        emit AIAnalysisAdded(_predictionId, _agentId, _credibilityScore);
    }

    function authorizeAgent(string memory _agentId) external onlyOwner {
        authorizedAgents[_agentId] = true;
    }

    function getAIAnalyses(uint256 _predictionId) external view validPrediction(_predictionId) returns (AIAnalysis[] memory) {
        return aiAnalyses[_predictionId];
    }
}
